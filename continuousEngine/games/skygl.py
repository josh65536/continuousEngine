...
from ctypes import c_float, c_ushort, c_int
...
class Sky(Game):
    VERTEX_SHADER = """
        ...
        uniform vec2 center;
        uniform vec2 half_dims; // vertical dim is negative because OpenGL's y axis goes up
        ...
        void main() {
            f_color = v_color;
            gl_Position = vec4((v_position - center) / half_dims, 0.0, 1.0);
        }
        """
    ...
    VIS_VERTEX_SHADER = """
        ...
        void main() {
            ...
            f_position = center + v_position * half_dims;
        }
    """
    ...
    VIS_FRAGMENT_SHADER = """
        ...
        void main() {
            if (f_position.x > 0 && f_position.x < N && f_position.y > 0 && f_position.y < N) {
                discard;
            }

            // color = vec4(f_position / 8.0 + vec2(0.5, 0.5), 0.0, 1.0);
            ...
            for (int ly = 0; ly < N + 1; ++ly) {
                for (int lx = 0; lx < N + 1; ++lx) {
                    ...
                    vec2 start = step(lat, f_position) * N;
                    vec2 inc = sign(lat - f_position);

                    // Find grid intersection points in order

                    for (int i = 0; i < N + 1; ++i) {
                        ts_vec[i] = (start + inc * i - f_position) / (lat - f_position);
                    }
                    ...
                    // First intersection is out of bounds; skip
                    if (y_i >= N + 1 || (x_i < N + 1 && ts_vec[x_i].x < ts_vec[y_i].y)) {
                        ++x_i;
                    } else {
                        ++y_i;
                    }

                    // Last intersection is out of bounds, and 2nd-to-last intersection
                    // hits an out-of bounds building, so don't calculate those.

                    for (int i = 1; i < N * 2; ++i) {
                        vec2 inter;

                        if (y_i >= N + 1 || (x_i < N + 1 && ts_vec[x_i].x < ts_vec[y_i].y)) {
                            inter = vec2(start.x + inc.x * x_i, f_position.y + (ly - f_position.y) * ts_vec[x_i].x);
                            ++x_i;
                        } else {
                            inter = vec2(f_position.x + (lx - f_position.x) * ts_vec[y_i].y, start.y + inc.y * y_i);
                            ++y_i;
                        }
                        
                        // Purify point
                        vec2 frac = inter - round(inter);
                        inter -= step(abs(frac), vec2(1e-4, 1e-4)) * frac;

                        ivec2 building = ivec2(
                            int(f_position.x < lx ? floor(inter.x) : ceil(inter.x) - 1),
                            int(f_position.y < ly ? floor(inter.y) : ceil(inter.y) - 1)
                        );
                        if (building.x >= 0 && building.x < N && building.y >= 0 && building.y < N) {
                            float s = solution[building.y * N + building.x] / distance(inter, f_position);
                            if (s > slope) {
                                slope = s;
                                visible[building.y * N + building.x] = 1;
                            }
                        }
                    }
                }
            ...
            color = vec4(num_visible / 16.0, num_visible / 16.0, 1.0, 1.0);
        }
    """
    ...
    def __init__(self, **kwargs):
        super().__init__(center=Point(n/2,n/2), spread=n, name='sky', gl=True, **kwargs)
        ...
        self.vis_vertex_shader = shaders.compileShader(self.VIS_VERTEX_SHADER, GL_VERTEX_SHADER)
        self.vis_fragment_shader = shaders.compileShader(self.VIS_FRAGMENT_SHADER.replace("#[n]", str(n)), GL_FRAGMENT_SHADER)
        self.vis_program = shaders.compileProgram(self.vis_vertex_shader, self.vis_fragment_shader)

        self.vertex_shader = shaders.compileShader(self.VERTEX_SHADER, GL_VERTEX_SHADER)
        self.line_fragment_shader = shaders.compileShader(self.LINE_FRAGMENT_SHADER, GL_FRAGMENT_SHADER)
        self.line_program = shaders.compileProgram(self.vertex_shader, self.line_fragment_shader)

        vis_vertices = [-1, -1, 1, -1, 1, 1, -1, 1]

        self.vis_vao = glGenVertexArrays(1)
        glBindVertexArray(self.vis_vao)

        self.vis_ibo = vbo.VBO((c_ushort * 6)(0, 1, 2, 2, 3, 0), target = GL_ELEMENT_ARRAY_BUFFER, size = 12)
        self.vis_ibo.bind()
        self.vis_vbo = vbo.VBO((c_float * len(vis_vertices))(*vis_vertices), size = len(vis_vertices) * 4)
        self.vis_vbo.bind()

        glEnableVertexAttribArray(0)
        glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, 8, self.vis_vbo)

        # Each vertex is an [x, y, r, g, b] structure
        grid_vertices = \
            [c for x in range(0, n+1) for c in [x, 0, 0, 0, 0, x, n, 0, 0, 0]] + \
            [c for y in range(0, n+1) for c in [0, y, 0, 0, 0, n, y, 0, 0, 0]]

        self.grid_vao = glGenVertexArrays(1)
        glBindVertexArray(self.grid_vao)

        self.grid_vbo = vbo.VBO((c_float * len(grid_vertices))(*grid_vertices), size = len(grid_vertices) * 4)
        self.grid_vbo.bind()
        
        glEnableVertexAttribArray(0)
        glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, 20, self.grid_vbo)
        glEnableVertexAttribArray(1)
        glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 20, self.grid_vbo + 8)
    ...
    def render(self):
        ...
        glUniform2f(glGetUniformLocation(self.vis_program, "center"   ), (self.x_max() + self.x_min()) / 2, (self.y_min() + self.y_max()) / 2)
        glUniform2f(glGetUniformLocation(self.vis_program, "half_dims"), (self.x_max() - self.x_min()) / 2, (self.y_min() - self.y_max()) / 2)
        glUniform1iv(glGetUniformLocation(self.vis_program, "solution"), n * n, (c_int * (n * n))(*[h for row in solution for h in row]))
        glBindVertexArray(self.vis_vao)
        glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_SHORT, self.vis_ibo)

        # Grid
        glUseProgram(self.line_program)

        glUniform2f(glGetUniformLocation(self.line_program, "center"   ), (self.x_max() + self.x_min()) / 2, (self.y_min() + self.y_max()) / 2)
        glUniform2f(glGetUniformLocation(self.line_program, "half_dims"), (self.x_max() - self.x_min()) / 2, (self.y_min() - self.y_max()) / 2)
        glBindVertexArray(self.grid_vao)
        glDrawArrays(GL_LINES, 0, (n + 1) * 4)
